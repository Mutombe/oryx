...
import { 
  ArrowRight, Play, ChevronRight, Check, Star, Quote,
  Shield, Award, Clock, Truck, Phone, Mail, MapPin,
  Home as HomeIcon, DoorOpen, Sparkles, MessageSquare,
  Users, CheckCircle, Headphones, PhoneCall, ArrowUpRight,
  Building2, Hammer, HardHat, Factory, ChevronLeft,
  Palette, Ruler, Settings, Wrench
} from 'lucide-react';
import { Button, Section, SectionHeader, StatCard, ProductCard, FeatureCard } from '../components/ui';
import { useSEO, generateOrganizationSchema, generateLocalBusinessSchema } from '../utils/seo';
import { 
  staggerContainer, staggerItem, fadeInUp, fadeInLeft, fadeInRight,
  scaleIn, heroTitle, heroSubtitle, cardHover
} from '../utils/animations';
import { companyInfo, products, services, stats, testimonials } from '../data/siteData';
        ...
        <div className="container-custom">
          <div className="grid lg:grid-cols-2 gap-6 lg:gap-8 items-center">
            ...
            >
              ...
              >
                <div className="inline-flex items-center gap-2 px-3 py-1.5 sm:px-4 sm:py-2 rounded-full bg-white/10 backdrop-blur-sm border border-white/10">
                  <div className="flex -space-x-1">
                    {[...Array(5)].map((_, i) => (
                      <GiBeveledStar key={i} className="w-3 h-3 sm:w-3.5 sm:h-3.5 fill-oryx-orange text-oryx-orange" />
                    ))}
                  </div>
                  <span className="text-xs sm:text-sm text-white/90">Trusted by 2000+ Clients</span>
                </div>
              </motion.div>
              ...
              >
                CRAFTING THE{' '}
                <span className="text-oryx-orange">FUTURE</span>{' '}
                IN STEEL
              </motion.h1>
              ...
              >
                Premium chromadek roofing materials, precision-engineered steel door frames, 
                and expert craftsmanship. Building Zimbabwe's tomorrow with quality that lasts generations.
              </motion.p>
              ...
              >
                {[
                  ...
                  <div key={idx} className="text-center">
                    <p className="text-2xl sm:text-3xl lg:text-4xl font-heading text-white">{stat.value}</p>
              ...
              >
                <div className="relative">
                  ...
                  <div className="relative bg-gradient-to-br from-oryx-orange to-oryx-orange-dark rounded-xl xl:rounded-2xl p-4 xl:p-5 text-center shadow-2xl">
                    <p className="text-3xl xl:text-4xl font-heading text-white">10+</p>
                    <p className="text-[10px] xl:text-xs text-white/80 uppercase tracking-wider">Years of<br/>Excellence</p>
                  </div>
                </div>
              </motion.div>
            </div>
                  ...
                  <div className="px-3 py-3 rounded-xl bg-white/10 backdrop-blur-md border border-white/20">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div className="min-w-0">
                        <p className="font-semibold text-white text-xs truncate">{card.title}</p>
                        <p className="text-[10px] text-gray-400 truncate">{card.subtitle}</p>
                      </div>
                    </div>
                  </div>
                </motion.div>
      ...
      <div className="container-custom">
        <div className="grid lg:grid-cols-2 gap-10 lg:gap-16 items-center">
          ...
          >
              ...
              <div className="absolute bottom-4 left-4 right-4 sm:bottom-6 sm:left-6 sm:right-6">
                <div className="bg-white/10 backdrop-blur-md rounded-xl p-3 sm:p-4 border border-white/20">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-white font-semibold text-sm sm:text-base">Since 2015</p>
                      <p className="text-white/70 text-xs sm:text-sm">Building Excellence</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            >
              <div className="flex items-center gap-3 sm:gap-4">
                ...
                <div>
                  <p className="text-2xl sm:text-3xl font-display font-bold text-navy-900">10+</p>
          ...
          >
            ...
            <motion.h2 variants={staggerItem} className="text-2xl sm:text-3xl lg:text-4xl xl:text-5xl font-display font-bold text-navy-900 mb-4 sm:mb-6">
              Zimbabwe's Trusted Name in{' '}
              <span className="text-oryx-orange">Quality Steel</span>
            </motion.h2>

            <motion.p variants={staggerItem} className="text-sm sm:text-base lg:text-lg text-gray-600 mb-6 sm:mb-8 leading-relaxed">
              {companyInfo.description}
            </motion.p>

            {/* Features list */}
            <motion.div variants={staggerItem} className="space-y-3 sm:space-y-4 mb-6 sm:mb-8 text-left">
              {[
            ...
            <motion.div variants={staggerItem} className="flex justify-center lg:justify-start">
              <Button to="/about" rightIcon={<ArrowRight className="w-4 h-4 sm:w-5 sm:h-5" />} className="text-sm sm:text-base">
                Learn More About Us
              </Button>
            </motion.div>
          </motion.div>
    ...
    <Section background="navy" className="py-16 sm:py-20 lg:py-24">
      <div className="container-custom">
        ...
        <motion.div
          ...
          className="text-center mt-8 sm:mt-10 lg:mt-12"
        >
          <Button to="/services" variant="white" rightIcon={<ArrowRight className="w-4 h-4 sm:w-5 sm:h-5" />} className="text-sm sm:text-base">
              ...
              <div className="h-full bg-white rounded-xl lg:rounded-2xl p-5 sm:p-6 lg:p-8 shadow-lg border border-gray-100 hover:shadow-xl hover:border-oryx-orange/20 transition-all duration-300">
                ...
                <div className="flex gap-1 mb-3 sm:mb-4">
                  {[...Array(testimonial.rating)].map((_, i) => (
                    <GiBeveledStar key={i} className="w-4 h-4 sm:w-5 sm:h-5 fill-oryx-orange text-oryx-orange" />
                  ))}
                </div>
                ...
                <div className="flex items-center gap-3 sm:gap-4 pt-4 sm:pt-6 border-t border-gray-100">
                  ...
                  <div className="min-w-0">
                    <p className="font-semibold text-navy-900 text-sm sm:text-base truncate">{testimonial.name}</p>
                    <p className="text-xs sm:text-sm text-gray-500 truncate">{testimonial.role}, {testimonial.company}</p>
                  </div>
                </div>
              </div>
            </motion.div>
      ...
      <div className="container-custom">
        ...
        >
          ...
          <p className="text-sm sm:text-base lg:text-xl text-white/90 mb-8 sm:mb-10 leading-relaxed">
            ...
            We're here to help bring your vision to life with quality materials.
          </p>
          <div className="flex flex-col sm:flex-row justify-center gap-3 sm:gap-4">
            <Button to="/contact" variant="white" size="lg" rightIcon={<ArrowRight className="w-4 h-4 sm:w-5 sm:h-5" />} className="text-sm sm:text-base justify-center">
              Get a Free Quote
            </Button>
            <Button 
              href={`tel:${companyInfo.contact.phones[0].replace(/\s/g, '')}`}
...
const Home = () => {
  // SEO
  useSEO({
    title: 'Oryx Steel Industries | Premium Roofing Materials & Steel Door Frames Zimbabwe',
    description: companyInfo.description,
    keywords: 'roofing materials Zimbabwe, chromadek sheets, steel door frames, Q-Tiles Harare, IBR sheets, laser cutting Zimbabwe'
  });
